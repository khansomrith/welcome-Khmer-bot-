...
// Admin user IDs (bypass permission check) - from .env
...
const commandLogWebhook = process.env.COMMAND_LOG_WEBHOOK
  ? new WebhookClient({ url: process.env.COMMAND_LOG_WEBHOOK })
  : null;

// Log command to webhook
async function logCommand(interaction) {
  if (!commandLogWebhook) return;
  ...
  try {
    ...
    await commandLogWebhook.send({ embeds: [embed] });
  } catch (error) {
  ...
  new SlashCommandBuilder()
    ...
    .addSubcommand((sub) =>
      sub
        .setName("view")
        .setDescription(
          "មើលតួនាទីស្វ័យប្រវត្តិបច្ចុប្បន្ន (View current auto role)",
        ),
    ),
  ...
  if (settings.enabled.welcome && settings.welcomeChannel) {
    try {
      ...
      if (channel) {
        ...
        const formattedMessage = replaceVariables(
          message,
          member,
          member.guild,
        );
...
client.on("guildMemberRemove", async (member) => {
  ...
  if (settings.enabled.leave && settings.leaveChannel) {
    try {
      ...
      if (channel) {
        const message =
          settings.leaveMessage || "លាហើយ {user.username} យើងនឹងចង់ចាំអ្នក! 👋";
        const formattedMessage = replaceVariables(
          message,
          member,
          member.guild,
...
client.on("interactionCreate", async (interaction) => {
  ...
  // Check permissions (allow admin IDs)
  ...
  if (
    ...
    return interaction.reply({
      content:
        "❌ អ្នកត្រូវការសិទ្ធិ Administrator ដើម្បីប្រើប្រាស់ commands ទាំងនេះ!",
      ephemeral: true,
    });
  }
      ...
      case "autorole": {
        ...
        if (subcommand === "set") {
          ...
          const embed = new EmbedBuilder()
            ...
            .setDescription(
              `តួនាទីស្វ័យប្រវត្តិ: ${role.toString()}\nស្ថានភាព: 🟢 បើក`,
            )
            .setTimestamp();
          ...
          const currentRole = settings.autoRole
            ? guild.roles.cache.get(settings.autoRole)
            : null;
      ...
      case "setup": {
        ...
        // View settings
          ...
          const welcomeCh = settings.welcomeChannel
            ? guild.channels.cache.get(settings.welcomeChannel)?.toString() ||
              "មិនឃើញ"
            : "មិនបានកំណត់";
          const leaveCh = settings.leaveChannel
            ? guild.channels.cache.get(settings.leaveChannel)?.toString() ||
              "មិនឃើញ"
            : "មិនបានកំណត់";
